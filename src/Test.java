 ...
 * Можно будет использовать только один объект класса Test
 */
public class Test {
    // В этой статической(!) переменной будет хранится единственный объект класса Test
    ...
    // Мы не хотим, чтобы пользователь мог создавать новые объекты класса Test
    private Test(String value) {
        this.value = value;
    }

    // Вот этот статический метод будет вызывать пользователь, чтобы получить объект класса Test
...
class SingletonTest {
    public static void main(String[] args) {
        Test first = Test.getInstance("Hello!");
        Test second = Test.getInstance("Goodbye!");
